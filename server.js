  ...
  for (let ci = 0; ci < CROSSINGS.length; ci++) {
    ...
    for (let ri = 0; ri < crossing.routes.length; ri++) {
      ...
      try {
        ...
        const row    = { timestamp: new Date().toISOString(), trafficMin: result.trafficMin, normalMin: result.normalMin, distanceKm: result.distanceKm, delay, level };
        ...
        ok(`[${crossing.shortName}] ${route.name}: ${result.trafficMin}min  delay:+${delay}  ${lc}${level}\x1b[0m`);
      } catch(e) {
...
function buildMainPage() {
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;border-top:1px solid var(--border)">
          ${[l0,l1].map((l,ri)=>{
            ...
            return `<div style="padding:16px 20px;${ri===1?'border-left:1px solid var(--border)':''}">
              ...
              <div class="metric-unit">${l?(l.delay>0?'+'+l.delay+' min delay':'no delay'):'waiting...'}</div>
            </div>`;
          }).join('')}
        </div>
  ...
  <div class="card">
    ...
    <div style="overflow-x:auto"><table>
      <thead><tr><th>Time</th><th>Crossing</th><th>Direction</th><th>Traffic</th><th>Normal</th><th>Delay</th><th>Status</th></tr></thead>
      <tbody>${(()=>{
        const all=[];
        ...
        return all.sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).slice(0,40)
          .map((r,i)=>`<tr style="${Math.floor(i/2)%2?'background:#f9f9f7':''}">
            ...
            <td>${r.trafficMin} min</td><td>${r.normalMin} min</td>
...
<footer>Nicosia Crossings Monitor · Every ${CONFIG.intervalMin} min · TomTom</footer></body></html>`;
}
...
function buildDetailPage(id) {
      ...
      return `<div class="card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;padding:18px 22px;border-bottom:1px solid var(--border)">
          <div>
            ...
            <div class="metric-big" style="color:${col}">${l.trafficMin}</div>
            <div class="metric-unit">minutes</div>
          </div>
          <div style="border-left:1px solid var(--border);padding-left:16px">
            <div class="metric-label">Normal</div>
            <div class="metric-big" style="color:var(--blue)">${l.normalMin}</div>
            <div class="metric-unit">minutes</div>
          </div>
          <div style="border-left:1px solid var(--border);padding-left:16px">
            <div class="metric-label">Delay</div>
            <div class="metric-big" style="color:${col}">${l.delay>0?'+':''}${l.delay}</div>
            <div class="metric-unit">minutes</div>
          </div>
        </div>
  ...
  <div class="card">
    ...
    <div style="overflow-x:auto"><table>
      <thead><tr><th>Time</th><th>Direction</th><th>Traffic</th><th>Normal</th><th>Delay</th><th>Status</th></tr></thead>
      <tbody>${(()=>{
        const all=[];
...
<footer>Nicosia Crossings Monitor · Every ${CONFIG.intervalMin} min · TomTom</footer>
...
function startServer() {
  http.createServer((req,res)=>{
    ...
    if(url==='/api/data'){
      ...
      res.end(JSON.stringify({crossings:CROSSINGS.map((c,ci)=>({id:c.id,name:c.name,routes:c.routes.map((r,ri)=>({name:r.name,latest:readings[ci][ri][readings[ci][ri].length-1]||null}))}))}));
      return;
    }
