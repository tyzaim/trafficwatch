...
// Calculate shared max across both directions for this crossing
const allValues = chartData.flatMap(d=>[...d.traffic,...d.normal]).filter(v=>v!=null);
const sharedMax = allValues.length ? Math.ceil(Math.max(...allValues) * 1.15) : 20;
...
chartData.forEach((d,idx)=>{
  ...
  new Chart(canvas,{
    ...
    data:{labels:d.labels,datasets:[
      {label:'With Traffic',data:d.traffic,borderColor:'#dc2626',backgroundColor:'rgba(220,38,38,0.04)',borderWidth:1,pointRadius:0,pointHoverRadius:3,fill:true,tension:0.3},
      {label:'Normal',data:d.normal,borderColor:'#2563eb',backgroundColor:'transparent',borderWidth:0.75,borderDash:[4,4],pointRadius:0,pointHoverRadius:3,fill:false,tension:0.3}
    ]},
    options:{responsive:true,maintainAspectRatio:false,
      ...
      scales:{
        ...
        y:{ticks:{color:'#8a8a85',font:{family:'DM Mono',size:10}},grid:{color:'#f0f0ee'},beginAtZero:true,min:0,max:sharedMax}
      }
    }
  });
});
