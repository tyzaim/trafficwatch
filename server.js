...
function buildDashboard() {
  ...
  const chartData = routeData.map(r => ({
    labels:  r.readings.slice(-24).map(x => new Date(x.timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})),
    traffic: r.readings.slice(-24).map(x => x.trafficMin),
    normal:  r.readings.slice(-24).map(x => x.normalMin),
  }));
...
<div class="header">
  ...
  <div class="header-right">
    <div class="last-updated">Updated: ${new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit', second:'2-digit'})}</div>
  ...
  <div class="routes-grid">
    ${routeData.map((r, idx) => {
      ...
      <div class="chart-area">
        <div class="chart-label">Travel time — last ${Math.min(r.readings.length, 24)} checks</div>
